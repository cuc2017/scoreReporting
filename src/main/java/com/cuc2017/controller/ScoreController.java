...
public class ScoreController {
	...
	public String createGame(@RequestParam("division") Long divisionId, @RequestParam("homeTeam") Long homeTeamId,
		...
		try {
			...
			List<Player> homeTeamPlayers = getGameService().getPlayers(game.getHomeTeam());
			for (Player player : homeTeamPlayers) {
				log.info(player.toString());
			}
			model.addAttribute("homeTeamPlayers", homeTeamPlayers);
			List<Player> awayTeamPlayers = getGameService().getPlayers(game.getAwayTeam());
			model.addAttribute("awayTeamPlayers", awayTeamPlayers);
